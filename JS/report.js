...
const API_VENTAS_MENSUAL = 'http://127.0.0.1:8000/api/ventas/reporte-ventas-mensual/';
const API_TOP_CLIENTES = 'http://127.0.0.1:8000/api/ventas/reporte-top-clientes/';
const API_TOP_PRODUCTOS = 'http://127.0.0.1:8000/api/detalleVenta/top-productos/';
const API_CLIENTES = 'http://127.0.0.1:8000/api/Cliente/';
const API_VENTAS_METODO_PAGO = 'http://127.0.0.1:8000/api/ventas/buscar-por-metodo-pago';

// Mapas y elementos del DOM
const clientesMap = {};
...
async function fetchVentasPorMes() {
    ...
    try {
        const res = await fetch(API_VENTAS_MENSUAL, { headers });
...
async function fetchTopClientes() {
    ...
    try {
        const res = await fetch(API_TOP_CLIENTES, { headers });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        return json.reporte || [];
    } catch (error) {
...
async function fetchProductosMasVendidos() {
    ...
    try {
        const res = await fetch(API_TOP_PRODUCTOS, { headers });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json = await res.json();
        return json.reporte || [];
    } catch (error) {
...
async function cargarClientes() {
    ...
    try {
        ...
        console.log("Clientes cargados:", clientesMap);
...
async function fetchVentasPorMetodoPago(metodo) {
    ...
    try {
        const url = new URL(API_VENTAS_METODO_PAGO);
...
async function initCharts() {
    const [ventasMes, topClientes, productosMasVendidos] = await Promise.all([
        fetchVentasPorMes(),
        fetchTopClientes(),
...
async function init() {
    await cargarClientes(); // Es importante cargar los clientes primero para que el mapa esté listo
    setupEventListeners();
    await initCharts();
}
